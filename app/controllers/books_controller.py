...
author_service = author_service.AuthorService(db.session)
...
def create():
    if request.method == 'POST':
        book_service.create_book(request.form['title'], request.form['author_id'])
...
def edit(book_id):
    ...
    authors = author_service.get_all_authors()
    if request.method == 'POST':
        book_service.update_book(book, request.form['title'], request.form['author_id'])
        return redirect(url_for('books.list'))
